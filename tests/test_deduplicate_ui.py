...
import tagger.deduplicate as deduplicate
import tagger.main_window as main_window
from tagger.deduplicate import DeduplicateDialog
from tagger.deduplication import DuplicatePair, DuplicateScanResult, KeepChoice
from tagger.delete_filter import DeleteFilterCommitResult
from tagger.domain import ImageEntry
from tagger.preview import PreviewLoader
from tagger.settings import USE_UNLINK_FOR_DEDUPLICATE_SETTING
...
def test_main_window_opens_deduplicate_and_refreshes_after_commit(
    ...
    assert not window.deduplicate_action.isEnabled()
    window._load_directory(tmp_path, show_issues=False)
    assert window.deduplicate_action.isEnabled()
    opened = []
    ...
    monkeypatch.setattr(main_window, "DeduplicateDialog", FakeDialog)
    window.deduplicate_action.trigger()
